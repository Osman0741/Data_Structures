...
public class HeapClassWithHeapify {

    int[] items;
    int size;

    public HeapClassWithHeapify(int capacity) {
        this.items = new int[capacity];
        this.size=0;
    }

    public HeapClassWithHeapify(int[] arr) {
    ...
    public void heapify(int index){
        ...
        if(!isValidParent(index)){
            ...
            heapify(largestChildIndex);  // recursively go down on the affected sub tree
